...
| **ALWAYS-mode 261 s -> 3.1 s on the 4-page math paper (v0.4.7)** - root causes found by per-region probe (`examples/probe_regions.rs`): (1) at our 1024-side letterbox DocLayout-YOLO emits only column/half-page `isolate_formula` boxes over ordinary text (upstream infers at native page resolution, ~8x pixels; BGR-vs-RGB ruled out experimentally), and each bogus box triggered a full ~1024-token TexTeller decode (~30 s apiece); (2) `"formula"` substring also matched `formula_caption` (a text label). Fixes: caption/footnote labels route to text first; implausible formula regions (>25% page height or >15% page area) degrade to text layer with a warning. (The proposed native-resolution follow-up was subsequently measured and REFUTED - see the layout resolution sweep line: the wide boxes are genuine model predictions and native resolution is slower with lower confidence.) Known tradeoff: layout-route formula detection on born-digital pages relies on v0.4.8's hybrid refinement - SURGICAL mode (text-layer heuristics) remains the formula path |
| **ALWAYS-mode hybrid formula refinement (v0.4.8)** - v0.4.7 bought the speed by throwing away ALL layout-route formulas on born-digital pages; this recovers the real ones at bounded cost. Layout `formula` regions are now refined by intersecting them with text-layer math boxes (`math_boxes_from_chars` with hgap 2.5 / vgap 0.9 - wider than SURGICAL's 1.5 because TeX `\quad` spacing fragments display runs; tighter vertically to never merge adjacent lines). Crops pass gates: width >= 40pt (labels/artifacts), display-style height 1.6-3x line height (merged blobs rejected), glyph-tight (no pad bleed). Decode budget is proportional to crop area (area/40, clamped 64-512 tokens - a 153x30pt sliver previously decoded 2600 garbage chars in 28 s at the 1024 default) and output must pass a plausibility filter (rejects `\mbox`/`\text` wrappers, English prose, >4x token repetition). Born-digital shortcut: a formula region with text-layer content but no math boxes provably has no equation -> plain text, zero decode (kills the remaining v0.4.7 residual where a sub-gate prose blob still triggered a full decode). Measured on the 4-page math paper: 66.7 s -> 14.6 s; the 2 real display equations recognized (one was a `\frac` in a prose paragraph that v0.4.7 dropped), 2 garbage decodes rejected by the filter, and ALWAYS output is byte-identical to SURGICAL on the fixture. Scans (no text layer) keep the v0.4.7 fallback: TexTeller on the region crop, now with the same area-proportional budget |
| **Layout resolution sweep - native resolution REFUTED (v0.4.8 measurement, `BOB_LAYOUT_MAXSIDE` probe, CPU)**: layout inferred at max-side 1024 / 1536 / 2048 / native (~3300 px) on the 4-page math paper. The half-page/column-wide `isolate_formula` boxes that v0.4.7 attributed to 1024-px letterbox inference appear at EVERY resolution (1024: 37 regions / 28 isolate_formula; 2048: 39/29; native: 36/29) - they are genuine DocStructBench predictions on dense math pages (the training set contains full-column equation blocks), not a letterbox artifact. Native resolution is strictly worse: layout latency 457 ms -> 5.5 s/page (12x) and confidences collapse (0.9+ -> 0.35-0.76). Conclusion: keep 1024 letterbox inference; wide formula regions must be handled downstream, which is exactly what v0.4.8's hybrid refinement does |
| **Per-slot provider control + auto-GPU defaults (v0.4.9)** - resolves the NOTE on the Rapid-GPU line above: one global `ort_providers` was taxing CUDA users on tables (measured 2-9x slowdown). `ConverterConfig` now exposes `encoder_ort_providers`, `decoder_ort_providers`, `layout_ort_providers`, `ocr_ort_providers`, `table_ort_providers` (all `Option<Vec<String>>`; `None` = default). Defaults encode the measured ledger: layout/OCR -> **auto-GPU** (when the loaded ONNX Runtime library registers the CUDA EP - once-locked probe, CPU-only libraries probe false at zero cost - CUDAExecutionProvider is prepended to the slot list; 12.3x / 3.6x with zero config); table -> **CPU-pinned** (SLANet 2-9x slower on CUDA); TexTeller -> base `ort_providers` (Int8+CUDA still guarded with a warning). End-to-end verified on the RTX 3090 (GPU dylib, all-default config): logs show auto-enable for the layout+OCR slots and the CPU-pinned table slot; 4-page ALWAYS run 14.6 s -> 11.4 s with TexTeller still on CPU. 83 unit + 9 integration tests green |
